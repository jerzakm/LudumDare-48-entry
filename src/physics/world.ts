...
export class WorldFactory {
  ...
  private createStaticPolygonAndChainShapes = (
  ...
  private createFallingShapes = (world: Box2D.b2World): void => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const shape = randomValue < 0.2 ? cshape : createRandomPolygonShape(20);
  ...
  private createRope = (): {
    ...
    const [wrappedMasses, destroyMasses]: [
      Box2D.WrapperObject,
      () => void
    ] = toFloatArray(masses);
    const [wrappedVertices, destroyVertices]: [
      Box2D.b2Vec2,
      () => void
    ] = pointsToVec2Array(vertices);
    ropeDef.set_masses(wrappedMasses);
    ropeDef.set_vertices(wrappedVertices);
    ropeDef.set_count(ropeLen);
    ropeDef.set_gravity(new b2Vec2(0, 0));
